...
- [ ] **GDTF model rendering in the visualizer (Phase 3).** Extract the embedded GLB meshes (trimesh), bake to indexed interleaved buffers with UVs, new textured shader variant, `MeshChassisGeometry` selected in `make_chassis_geometry()` building the kinematic chain from the GDTF geometry tree (pan/tilt at the Axis nodes, beam origin from the Beam node), mesh sharing across instances, per-OS mesh cache. Fallback ladder for wild-file quality: sane GLB -> mesh chassis; PrimitiveType-only / 3DS-only / failed validation -> existing procedural chassis; `.qxf` -> unchanged. Tests against self-authored `.gdtf` files (GDTF Share terms forbid bundling downloaded ones) plus golden screenshots.
