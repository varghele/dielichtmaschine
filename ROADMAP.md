...
- [ ] **GDTF definition import (Phase 1, includes the spike gate).** First pass shipped: `utils/gdtf_loader.py` transpiles GDTF (via `pygdtf`) into the canonical `FixtureDefinition` by synthesizing a QLC-format channel model - attribute-to-preset mapping, channel functions/sets to capability ranges, wheel slots with CIE-xyY-to-sRGB colors, 16-bit channels to coarse+fine pairs, `GeometryReference` cells to `<Head>` blocks, and physical data (dimensions, beam angle, pan/tilt ranges, LuminousFlux) from the geometry tree. `gdtf_fixtures/` is scanned first, so GDTF wins over `.qxf` on identity clashes; the browser tags `[GDTF]` entries. Proven end to end by tests: `.qxw` export, visualizer payload, capability detection all consume GDTF definitions unchanged (`tests/unit/test_gdtf_loader.py`, 11 tests, synthetic in-repo GDTF files only). **Open before ticking:** the spike gate - rebuild the five demo rigs from real GDTF Share definitions and write the capability-coverage decision note in `docs/`; needs Share downloads (user account, files cannot be committed).
