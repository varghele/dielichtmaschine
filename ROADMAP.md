...
- [ ] **Fixture-definition layer unification (Phase 0, prerequisite).** Today QXF is parsed in five independent places with five duplicated library-search implementations; live DMX and all export consume a different dict shape than the renderer's `FixtureCapabilities`. Collapse them into one `FixtureLibrary` service producing a canonical `FixtureDefinition` that every consumer uses. Pure refactor, behaviour preserving; acceptance is byte-identical `.qxw` export for the five demo rigs before vs after. Without this, GDTF would just be parser number six.
