...
- [x] **Stage layers (vertical stacking).** ~~Multiple Z-planes per stage (e.g. ground stack, mid-truss, top-truss).~~ Done: `StageLayer` (name, z_height, visible) on `Configuration` + `Fixture.layer`. Stage tab grew a Stage Layers panel (checkable list = visibility, add/edit/remove) and an "Assign to Layer" right-click submenu; assignment snaps the fixture's Z to the plane, editing the plane height moves its fixtures with it, removal keeps heights. Hidden layers are filtered once in `build_fixtures_payload`, so the 2D plot, all three embedded previews, and the standalone TCP visualizer treat them identically. Round-trips through config YAML and fixture-list CSV/JSON. Follow-up shipped: **active-layer editing** — double-click a layer (or press L to cycle) and only that layer's fixtures stay interactive; everything else ghosts to a faint locked reference for placement. 26 tests (`test_stage_layers.py` + layer cases in `test_fixture_io.py`).
