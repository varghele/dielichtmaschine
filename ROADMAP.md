...
- [ ] **Branding switch and UI rework.** Decided 2026-07-07: the product is **Die Lichtmaschine** (dielichtmaschine.de), accent Glutorange `#F0562E`, typography Barlow / Barlow Condensed / IBM Plex Mono, hard-edged datasheet aesthetic. The design North Star (all screens, tokens, brand assets) lives in `design_handoff_lichtmaschine_app/`; the rebrand execution plan with phase breakdown is `docs/rebranding-plan.md`. Scope of this item: identity (window title, org/app names + settings migration, About, `--version`), brand assets + shipped fonts, theme retokenization to the handoff palette (token dict rendered into one QSS template), PyInstaller spec + artifact names, README + docs, repo rename checklist. Alongside the rename, a framing pass to de-centre QLC+: `.qxw` export moves from headline feature to one interop option, native ArtNet playback becomes the front-and-centre workflow. Existing user configs and file formats stay compatible. Full screen redesigns from the North Star are NOT this item - they land with the milestone that owns each screen (timeline v1.6, live console v1.8, morph wizard v1.5b, stage plot symbols with the reopened v1.1 stage work).
- [ ] **Structured local log** (pulled forward from v1.4 so the rebrand and later UI work are self-reporting when they break something). Default-on file logging to the per-OS app-data dir of the new brand, rotated by day, with a "Reveal log folder" action in the Help menu. Captures unhandled exceptions, fixture-definition warnings, ArtNet socket failures, and the export pipeline's status.
- [ ] **Crash reporter dialog** (pulled forward from v1.4, same reasoning). On `sys.excepthook` / Qt unhandled exception, surface a modal with the traceback, the app version, and a "Copy to clipboard / Save as file" affordance the user can paste into a GitHub issue. No automatic upload.
