...
- [ ] **GDTF definition import (Phase 1, includes the spike gate).** GDTF loader via `pygdtf` producing the same canonical `FixtureDefinition`: attribute system into the shared semantic channel vocabulary, channel functions into capability ranges, wheels, `GeometryReference`/breaks into multi-head/cell emitters, physical data (dimensions, beam, LuminousFlux replacing the power-based lumens estimate) from the geometry tree. A `gdtf_fixtures/` user folder scanned alongside the QLC+ dirs; the fixture browser lists both formats and prefers GDTF when both exist. The original spike survives as the validation gate: rebuild the five demo rigs from GDTF, compare capability coverage against the `.qxf` parse, decision note in `docs/` before the switch is declared done.
