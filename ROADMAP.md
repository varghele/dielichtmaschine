...
- [ ] **GDTF persistence and interop (Phase 2).** `Fixture` gains a definition-source field (YAML schema bump, absent field defaults to `qxf`) plus GDTF FixtureTypeID + revision; `fixture_io` resolves GDTF too. `.qxw` export of GDTF-patched fixtures: exact-match to an existing QLC+ library `.qxf` first, else generate a companion `.qxf` from the GDTF definition, with an export report naming the mechanism per fixture. Needs real-QLC+ verification like the v1.0 runtime check.
