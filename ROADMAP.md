...
- [ ] **Branding switch and UI rework.** "QLC+ Show Creator" no longer describes a standalone product; the name, and a UI that reads as a QLC+ exporter frontend, both go. Scope: pick a new name (trademark / package-name / domain sanity check), then rename everything that carries the old one - window title, repo, PyInstaller spec + artifact names, per-OS data dirs (`%APPDATA%/QLCShowCreator/`, planned in v1.4), README + demo media, docs. Alongside the rename, a UI pass to de-centre QLC+: `.qxw` export moves from headline feature to one interop option among the export paths, native ArtNet playback becomes the front-and-centre workflow, and first-run / template flows stop assuming a QLC+ install. Existing user configs and file formats stay compatible; only the branding and framing change.
