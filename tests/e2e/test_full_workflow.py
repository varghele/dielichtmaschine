...
class TestStep3Stage:
    ...
    def test_set_orientation_context_action_opens_modal_and_writes_back(
        ...
        # 'standing' = pitch -90 (chassis upright), the pre-rebrand
        # convention restored 2026-07-13 - see utils/orientation.py
        # MOUNTING_PRESET_ANGLES.
        assert par1.pitch == pytest.approx(-90.0)
        assert par1.roll == pytest.approx(0.0)
...
class TestFullWorkflow:
    def test_author_play_and_round_trip(
        ...
        # 'standing' = pitch -90 (pre-rebrand convention); survives save +
        # load (the load-time orientation migration leaves it alone).
        assert pars[0].pitch == pytest.approx(-90.0)
        assert pars[0].roll == pytest.approx(0.0)
